...
public class MultiplexerTimeServer implements Runnable {
    ...
    public MultiplexerTimeServer(int port) {
        try {
            //TODO 步骤一、打开一个通道，用于监听客户端的连接，是所有客户端连接的父管道
            servChannel = ServerSocketChannel.open();
            //TODO 步骤二、绑定监听端口，设置连接为非阻塞模式
            //backlog = 1024        requested maximum length of the queue of incoming connections.
            servChannel.socket().bind(new InetSocketAddress(port), 1024);
            //设置连接为异步非阻塞模式
            servChannel.configureBlocking(false);
            //TODO 步骤三、创建Reactor线程，创建多路复用器并启动线程
            selector = Selector.open();

            //TODO 步骤四、将ServerSocketChannel注册到Reactor线程的多路复用器Selector上，监听ACCEPT事件
    ...
    public void run() {
        while (!stop) {
            try {
                //TODO 步骤五、多路复用器在线程run方法的无限循环体内轮询准备就绪的Key
